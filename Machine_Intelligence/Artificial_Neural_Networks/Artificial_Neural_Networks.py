...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        if(gradients is not None):
            ...
            shape1=self.history[1].shape[0]
            shape2=self.history[2].shape[1]
            ones_array= np.ones((shape1,shape2),dtype=int)
            array=np.array(ones_array)
            grad=array
    ...
    def grad_matmul(self, gradients=None):
        ...
        History1=self.history[1].arr
        History2=self.history[2].arr
        Transpose1= np.transpose(History2)
        Transpose2= np.transpose(History1)
        Result1 = np.matmul(grad,Transpose1)
        Result2 = np.matmul(Transpose2,grad)
    ...
    def backward(self, gradients=None):
        ...
        if(History0=='leaf'):
            ...
            if(History0=='matmul'):
                grad=self.grad_matmul(gradients)
            else:
                grad=self.grad_add(gradients)

        
        History1=self.history[1]
        History2=self.history[2]
        grad1=grad[0]
        grad2=grad[1]
        self=History1
        self.backward(grad1)
